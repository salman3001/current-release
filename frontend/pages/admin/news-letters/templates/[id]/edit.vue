...
watch(
  template,
  () => {
    form.value.template.name = (template.value as any)?.name || "";
    form.value.template.desc = (template.value as any)?.desc || "";
    form.value.template.content = (template.value as any)?.content || "";
  },
  { deep: true, immediate: true }
);

const { execute: createTemplate, loading: posting } = templateApi.put(
  {},
  {
    onSuccess: () => {
...
const submit = () => {
  ...
  createTemplate(route.params.id as string, formData);
};
</script>
