...
const { data: category } = await blogCategoryApi.show(route.params.id as string)
watch(category, () => {
  form.value.name = (category.value as any)?.name;
  form.value.slug = (category.value as any)?.slug;
  form.value.languageId = (category.value as any)?.language_id;
  form.value.order = (category.value as any)?.order;
  form.value.status = (category.value as any)?.status == 1 ? true : false;
  form.value.metaTitle = (category.value as any)?.meta_title;
  form.value.metaKeywords = (category.value as any)?.meta_keywords;
  form.value.metaDesc = (category.value as any)?.meta_desc;
}, {
  deep: true,
  immediate: true
})
