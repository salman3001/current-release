...
const { data: category } = await serviceSubCategoryApi.show(route.params.id as string, {
  populate: {
    faqs: {
      fields: ['*']
    },
...
watch(category, () => {
  form.value.category.longDesc = (category.value as any)?.long_desc || ''
  form.value.category.name = (category.value as any)?.name || ''
  form.value.category.shortDesc = (category.value as any)?.short_desc || ''
  form.value.category.status = (category.value as any)?.status == 1 ? true : false
  //seo
  form.value.seo.metaDesc = (category.value as any)?.seo?.meta_desc || ''
  form.value.seo.metaKeywords = (category.value as any)?.seo?.meta_keywords || ''
  form.value.seo.metaTitle = (category.value as any)?.seo?.meta_title || ''
  //faq
  form.value.faq = (category.value as any)?.faqs ? (category.value as any)?.faqs?.map((f: any) => ({ quest: f.quest, ans: f.ans })) : []
...
const { execute: createCategory, loading: posting } =
  serviceSubCategoryApi.put({}, {
    onSuccess: () => {
      navigateTo(routes.admin.service.sub_category.index)
    }
  });

const submit = () => {
  const formData = convertToFormData(form.value)
  createCategory(route.params.id as string, formData)
};
      ...
      <div class="q-gutter-y-sm">
        ...
        <div class="row q-col-gutter-md">
          <div class="col-12 q-mb-xl">
            ...
            <FormsImageInput name="image" @image="(f) => { form.image = f as unknown as null }" style="max-width: 15rem;"
              :url="category?.thumbnail?.url && $config.public.baseApi + category?.thumbnail?.url" />
          </div>
