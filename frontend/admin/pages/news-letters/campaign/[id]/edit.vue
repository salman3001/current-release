...
import { campaignTypeApi, interestApi, templateApi, campaignApi } from '@/utils/BaseApiService';
...
const { data: templates } = await templateApi.index({
  fields: ['name', 'id']
})
...
watch(campaign, () => {
  form.value.campaign.campaignTypeId = (campaign.value as any).campaign_type_id || ''
  form.value.campaign.deliveryDateTime = (campaign.value as any).delivery_date_time ? formatDate((campaign.value as any).delivery_date_time, 'DD/MM/YYYY HH:mm') : ''
  form.value.campaign.fromEmail = (campaign.value as any).from_email || ''
  form.value.campaign.fromName = (campaign.value as any).from_name || ''
  form.value.campaign.name = (campaign.value as any).name || ''
  form.value.campaign.replyTo = (campaign.value as any).reply_to || ''
  form.value.campaign.status = (campaign.value as any).status == 1 ? true : false
  form.value.campaign.subject = (campaign.value as any).subject || ''

  form.value.interests = (campaign.value as any).interests ? (campaign.value as any).interests.map((i: any) => i?.id) : []
  form.value.templateId = (campaign.value as any)?.template?.id || ''

  template.value = (campaign.value as any)?.template || null
}, {
  immediate: true
})
