...
<template>
    <div>
        <q-item :class="$q.screen.lt.sm ? 'column items-start q-gutter-sm' : 'row'">
            ...
            <q-item-section>
                <div v-if="discount.gt(0) && variant.discount_type === DiscountType.FLAT"><q-badge
                        color="negative">&#x20B9;{{
            variant.discount_flat }} off</q-badge></div>
                <div v-if="discount.gt(0) && variant.discount_type === DiscountType.PERCENATAGE"><q-badge
                        color="negative">{{
            new BigNumber(variant.discount_percentage).toFixed() }}% off</q-badge></div>
    ...
    <div class="row justify-end  text-bold text-h6">
        Total: &#x20B9;{{ new BigNumber(variant.price).minus(discount).times(qty).toFixed() }}
    </div>
    <div class="row justify-end text-bold text-h6">
        <q-btn color="primary" @click="getSummary" :disabled="bookingStore.loadingSummary">
            <LoadingIndicator v-if="bookingStore.loadingSummary" /> Proceed to checkout
        </q-btn>
    </div>
</template>