  ...
  >
    <q-card-section>
      <div class="row q-gutter-y-md wrap justify-between">
        ...
        <div
          v-if="accepted"
          class="column col-12 col-sm-4 gap-50"
    ...
    <q-card-section :horizontal="$q.screen.gt.xs">
      <q-card-section
        class="full-width"
        :class="$q.screen.gt.xs ? ' ' : 'q-pa-none'"
      >
    ...
    <q-card-actions class="justify-end q-pt-none q-gutter-sm">
      <q-btn
        color="primary"
        v-if="!anyBidAccepted"
        :disabled="anyBidAccepted || loading"
        @click="
          modal.togel('confirm', {
            title: 'Accept Bid',
            message:
              'You Are about to accept this bid. After accepting you will be able to intract with vendor. You can reject any time',
            iconName: 'info',
            onAccept: () => {
              acceptBid();
            },
      ...
      <NuxtLink :to="routes.book_custom_Service(requirementId)">
        <q-btn color="primary" v-if="accepted"> Confirm and book </q-btn>
      </NuxtLink>
    </q-card-actions>
  </q-card>
</template>
