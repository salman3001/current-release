...
<template>
  <q-card class="my-card relative-position" flat>
    ...
    <q-card-section>
      <div class="row justify-between items-center">
        <NuxtLink :to="routes.view_service(service?.id)">
          <div class="col text-h6 ellipsis text-nutral">{{ service?.name }}</div>
        </NuxtLink>
        <div class="row items-center q-gutter-xs">
          <q-icon name="star" color="primary" size="20px"></q-icon><span>{{ service.avg_rating }}</span>
        </div>
      </div>
    </q-card-section>
    ...
    <q-card-section class="q-pt-none">
      ...
      <div class="text-subtitle1 text-bold">{{ service.variants?.length ? 1 && "Starting From" : '' }} &#x20B9;{{
        service.variants?.length > 1 ? service?.starting_from : service?.variants[0]?.price }}</div>
    </q-card-section>
