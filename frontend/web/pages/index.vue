...
const { query, getServiceList } = useServiceApi.list({
...
} = await useAsyncData(() => getServiceList(), {
  watch: [query],
  deep: true,
});
...
<template>
  ...
  <div class="row justify-between items-center q-col-gutter-md">
    ...
    >
      <div
        class="row gap-50 justify-center"
        style="width: max-content; flex-wrap: nowrap"
      >
        <WebCategoryIcon
          v-for="c in data?.categories"
          :category="c"
          :selected="(route.query.tab as unknown as number) == c.id ? true : false"
          @select="(v: number) => {
        navigateTo({
          path: routes.home,
    ...
    <div
      class="row justify-end gap-50 col-12 col-sm-1"
