...
<template>
  <q-card bordered class="q-gutter-y-none full-width shadow-16">
    <q-toolbar class="justify-between q-gutter-md">
      ...
      <div>
        ...
        <ClientOnly>
          ...
          <div class="row" :class="$q.screen.xs ? 'justify-end' : 'justify-end'">
            <q-badge class="q-badge-warning text-body2 q-px-md" outline
              v-if="!requirement.accepted_bid_id">Active</q-badge>
            <q-badge class="q-badge-positive text-body2 q-px-md" outline
              v-else-if="requirement.accepted_bid_id != null">Accepted</q-badge>
            <q-badge class="q-badge-negative text-body2 q-px-md" outline v-else-if="date.getDateDiff(
    ...
    <q-card-section class="row items-center justify-between q-gutter-y-md">
      ...
      <div class="row items-center q-gutter-sm">
        <q-badge class="text-body2 q-px-md q-badge-info q-px-lg q-py-xs"><q-icon name="messages" /> &nbsp;Bids&nbsp;
          <span class="text-bold text-body1">{{
          requirement?.meta?.recievedBids_count
        }}</span></q-badge>
        ...
        <q-badge class="text-body2 q-px-sm q-badge-negative q-px-lg q-py-xs"><q-icon name="done" />&nbsp;Accepted
          Bid&nbsp;
          <span class="text-bold text-body1">
            {{ requirement.accepted_bid_id ? 1 : 0 }}</span></q-badge>
        <NuxtLink :to="routes.view_service_requirement(requirement.id)"
          v-if="route.fullPath != routes.view_service_requirement(requirement.id)">
