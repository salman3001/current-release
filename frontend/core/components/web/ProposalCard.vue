...
<template>
  ...
  >
        ...
        <div class="row gap-50">
          ...
          <div>
            <p v-if="accepted" class="text-bold text-subtitle1">
              ...
              <q-btn size="xs" color="secondary" @click="emit('create-chat')"
                >chat</q-btn
              >
            </p>
    ...
    <q-card-action class="row q-gutter-sm q-px-sm justify-end">
      <!-- <q-btn
        color="primary"
        @click="emit('negotiate', bid)"
        v-if="
          bid?.negotiate_history?.length < 1 ||
          bid?.negotiate_history[bid?.negotiate_history?.length - 1]?.accepted
        "
        >Negotiate</q-btn
      > -->
