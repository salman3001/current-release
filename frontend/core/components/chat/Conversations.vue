...
if (route.query?.newConversationId) {
  const existingConversation = data.value?.data.filter(c => c.id == route.query.newConversationId as unknown as number)
  ...
  <template v-else v-for="c in conversationsRef?.data" :key="c.id">
    ...
    formatDistance(Date.now(), c.messages[0]?.created_at, {
      includeSeconds: false,
        ...
        <q-item-label v-if="c.messages[0]" caption class="text-muted ellipsis-2-lines">{{ c.messages[0].body
          }}</q-item-label>
        <q-item-label v-else caption class="text-muted ellipsis-2-lines text-nutral">send hi <q-icon
            name="waving_hand" /> to
          <span v-if="c?.participant_one_identifier != myIdentifier">
            {{
    c?.participantOne?.adminUser?.first_name ||
    ...
    <q-separator :color="selectedCoversationId === c.id ? 'primary' : ''"
