...
<template>
    <q-layout view="lHh lpR lff" container class="bg-nutral-dim" :style="{ height: $q.screen.gt.sm ? '90vh' : '85vh' }">
        <q-header bordered class="bg-nutral-dim">
            <q-toolbar flat class="text-nutral q-px-sm q-px-md-lg">
                ...
                <q-toolbar-title v-if="selectedConversation" class="row gap-50 items-center gap-100">
                    <q-avatar>
                        <img :src="getImageUrl(
        selectedConversation.participant_one_identifier !=
            myIdentifier
            ? selectedConversation?.participantOne?.user?.profile
                ?.avatar?.url
            : selectedConversation.participant_two_identifier !=
                myIdentifier
                ? selectedConversation?.participantOne?.user?.profile
                    ?.avatar?.url
                : undefined,
        ...
        <q-drawer bordered show-if-above side="left" v-model="leftDrawerOpen" class="" style="width: 450px">
            <ScrollArea width="100%" height="100%">
                <q-list padding>
                    <ChatConversations :new-message="newMessage" :selected-coversation-id="selectedConversation?.id"
                        @selected="(v) => {
    ...
    " />
        ...
        <q-page-container :style="{ height: $q.screen.gt.sm ? '89vh' : '85vh' }">
            <ChatMessages :newMessage="newMessage" :selected-conversation="selectedConversation!"
                v-if="selectedConversation" :socket="(socket as any)" />
            <div v-else class="row full-width justify-center items-center" style="height: 80%">
        ...
        <q-footer elevated class="bg-nutral text-white" v-if="selectedConversation">
            <div class="row">
                <q-input :rows="1" type="textarea" autogrow outlined v-model="message" color="primary"
                    class="rounded-borders" style="flex-grow: 5" placeholder="Type a message"
                    @keyup.enter="createMessage()">
                </q-input>

                <q-btn name="send" color="primary" outline icon="send" size="lg" style="flex-grow: 1; max-width: 100px"
                    @click="createMessage()" />
            </div>
        </q-footer>
    </q-layout>
