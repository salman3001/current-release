...
const { user } = useAuth() as { user: Ref<IVendorUser> };
...
const initialForm = {
  ...
  businessName: user.value.business_name,
};
...
<template>
  ...
  <q-form class="q-gutter-y-md" v-if="mode == 'write'" @submit="updateAccount">
    ...
    rules.email('Email is not valid'),
      ...
      <div class="col-12 col-sm-6 col-md-4">
        <div class="text-subtitle1 text-bold">Business Name</div>
        <q-input outlined v-model="form.businessName" dense :rules="[rules.required('Required')]" />
      </div>
      <div class="col-12 col-sm-6 col-md-4">
